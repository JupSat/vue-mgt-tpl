<template>
  <div class="tabs">
    <el-tabs :model-value="activeTab" type="card" class="menu-tabs" @tab-click="clickTab" @tab-remove="removeTab">
      <el-tab-pane v-for="item in tabs" :key="item.name" :name="item.name" :label="$t(firstCharChange(item))"
    ...
    <el-dropdown trigger="click" class="operate">
      ...
      <template #dropdown>
        <el-dropdown-menu>
          <el-dropdown-item v-for="item in dropdownMenuList" :disabled="item.disabled" @click="tabsClose(item.key)">
            {{ item.label }}
          </el-dropdown-item>
        </el-dropdown-menu>
      </template>
